...
setTimeout(() => {
    ...
    messageElement = document.getElementById('search-' + message)
...
Echo.private('users.{{ Auth()->User()->id }}')
 ...
 " --}} class="w-full overflow-hidden">
                    ...
                    @if ($message->conversation_id == $this->selectedConversation->id)
                        ...
                        <div wire:key="{{ time() . $key }}" @class([
                            ...
                            <div @class([
                                ...
                                <p :class="{ 'bg-gray-700 text-white p-1 rounded-lg': message == {{ $message->id }} && open ==
                                        true }"
                                    class="whitespace-normal truncate text-sm md:text-base tracking-wide lg:tracking-normal text-justify"
    ...
    {{-- <script >
    window.addEventListener('findMessage', event => {

    //   alert('message-' + event.detail.message_id)

   
   
   
    messageElement = document.getElementById('message-' + event.detail.message_id);
    if (messageElement) {
        window.livewire.emit('loadMore');
        messageElement.scrollIntoView({ 'behavior': 'smooth' });
        }
