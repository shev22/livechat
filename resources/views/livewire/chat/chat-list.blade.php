 ...
 class="flex flex-col transition-all h-full overflow-hidden">
    ...
    <header class="px-3 z-10 bg-white sticky top-0 w-full py-2">
        ...
        <div class="border-b justify-between flex items-center pb-2">
            ...
            <input type="text" class="  text-xs text-gray border-0 rounded-lg w-56 font-extrabold  text-gray-900"
        ...
        <div class="flex gap-3 items-center p-2 bg-white mt-3">
    ...
    <main class=" overflow-y-scroll overflow-hidden  grow  h-full relative " style="contain:content">
        ...
        @if ($search)
            ...
            <ul>
                @foreach ($searchResults as $key => $search)

                
                     <div class="p-1 grid w-full spacey-y-2 bg-gray-400">
                        <h3 class="font-bold ">{{ ucfirst($key) }}</h3>
                    </div>
                    @foreach ($search as $item)
                        <li class="bg-gray-100 hover:bg-gray-200 cursor-pointer" wire:click="message({{$item->id }})">
                            <p  @class(['font-bold text-sm mx-4', 'p-1' => $key == 'users'])>{{ $item->name }}</p>
                        </li>
                        <li class="bg-gray-100  hover:bg-gray-200 cursor-pointer" wire:click="findMessage({{$item->id }}, {{ $item->conversation_id }})">
                            <p @class([' text-base mx-2 text-justify text-sm', 'my-1  ' => $key == 'messages'])> <span class="font-black">{{$item->sender_id == Auth::id() ? 'You' : $item->sender_name }} {{ $key == 'messages' ? ':':'' }}</span> 
                                {!! str_replace($this->search,"<span class='bg-yellow-300 text-white font-bold  rounded p-1'>{$this->search}</span>",  $this->trimSearch($item->body, $this->search, 30 )    )  !!}  
                              {{-- {{ $this->trimSearch($item->body, $this->search, 300 )             }} --}}
                        </p> 
            ...
            @if ($conversations)
                @foreach ($conversations as $key => $conversation)
                    <li id="conversation-{{ $conversation->id }}" wire:key="{{ $conversation->id }}"
                        class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4  relative w-full cursor-pointer px-2 {{ $conversation->id == $selectedConversation?->id ? 'bg-gray-200/70' : '' }}">
                        ...
                        <aside class="grid grid-cols-12 w-full">
                            ...
                            <a href="{{ route('chat', $conversation->id) }}"
                                ...
                                <div class="flex justify-between w-full items-center">
                                    ...
                                    <h6 class="truncate font-medium tracking-wider text-gray-900 hover:text-white">
                                ...
                                <x-dropdown align="right" width="48">
                                    <x-slot name="trigger">
                                        <button>
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                fill="currentColor"
                                                class="bi bi-three-dots-vertical w-7 h-7 text-gray-700 hover:text-white"
                                                viewBox="0 0 16 16">
                                                <path
